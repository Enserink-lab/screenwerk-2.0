...
plot.dispensingData <- function(x, .saveto, ...){
  ...
  # Removal of untreated and backfillings for single drug treatments
  ...
  dispPlot <- do.call(rbind, setNames(lapply(split(dispPlot, dispPlot$Combination.ID),
                                             function(x) if(nrow(x) >= 2 & any(x$Drug %in% dispensingData[["dataList"]][["ctrlList"]]$NAME) & !all(x$Drug %in% dispensingData[["dataList"]][["ctrlList"]]$NAME)){
                                               subset(x, !(Drug %in% dispensingData[["dataList"]][["ctrlList"]]$NAME))}else{x}), NULL))
  ...
  # Controls are labelled with only the drug name, unless there are multiple concentrations for a given control,
  # then also the drug concentration will be added
  dispPlot <- within(dispPlot, Drug <- ifelse(Drug %in% c("BzCl", "DMSO"), paste(Drug, sep = " "), paste(Drug, Drug.Concentration, sep = " ")))
  dispPlot <- within(dispPlot, Drug <- ifelse(Drug == "DMSO" & length(grep("DMSO",  dispensingData[["dataList"]][["ctrlList"]]$NAME)) > 1, paste(Drug, Drug.Concentration, sep = " "), paste(Drug, sep = " ")))
