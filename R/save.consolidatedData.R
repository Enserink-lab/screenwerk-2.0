...
#' 
#' @details This function extends the generic function \code{\link{save}} for objects of class 'consolidatedData'. It exports the consolidated data file for use outside the scope of metascreen. 
#' 
...
save.consolidatedData <- function(x, .saveto, ..., .fileformat, .format = FALSE, .sep){
  ...
  if(!file.exists(file.path(.saveto))){ dir.create(file.path(dirname(.saveto)), showWarnings = FALSE, recursive = TRUE) }
  if(!utils::file_test("-d", dirname(.saveto))){stop("in '.saveto'. Argument needs to be a valid folder location.", call. = TRUE)}
  # Check, if folder location includes a filename.
  if(grepl(".", sub(".*\\/", "", .saveto), fixed = TRUE)){.filename = basename(.saveto)}
  ...
  if(any(.format == "breeze", grepl("breeze", .format, ignore.case = TRUE))){
    ...
    utils::write.table(dfs.breeze, file = file.path(dirname(.saveto), ifelse(exists(".filename"), .filename, paste("BREEZE data file", ".", .fileformat, sep = ""))), sep = ",", dec = ".", row.names = FALSE, quote = FALSE)
    
  } else {
    
    cat('\r', "> Saving consolidated file... ", strrep(" ", 100), sep = "")
    
    utils::write.table(.consolidatedData, file = file.path(dirname(.saveto), ifelse(exists(".filename"), .filename, paste("Consolidated data file", ".", .fileformat, sep = ""))), row.names = FALSE, quote = FALSE, sep = .sep)
