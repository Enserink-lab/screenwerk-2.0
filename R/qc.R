...
#' @param .saveto \code{string}; path to a folder location where the object is saved to.
#' @param .ctrls \code{list}; a list of controls to be analyzed based on a set of positive and negative controls.
...
#' 
#' 
#' With the parameter \emph{.ctrls}, it is possible to specify a list with a set of positive and negative controls for (QC) analysis. The \strong{positive} control needs to be provided with a single positive control, while the \strong{negative} control can be a set with any number of negative controls. Alternative the \strong{negative} control can be set with \strong{all} leading to the inclusion of all negative controls present in the drug sensitivity screen. For detailed examples, see section 'Examples'.
#' 
#' 
...
#' 
#' @importFrom ggplot2 ggplot aes facet_grid geom_boxplot geom_point ggtitle scale_fill_continuous scale_y_continuous scale_colour_manual
...
qc <- function(consolidatedData, .saveto, .ctrls=list(positive, negative), .qcMethod){
  ...
  # Check, if a set of controls have been provided, otherwise retrieve controls from the consolidated dispensing data
  if(missing(.ctrls)){
    if(is.null(consolidatedData[["dispensingData"]][["dataList"]][["ctrlList"]])){
      stop("in '.ctrls'. Please provide a set of controls to analyse.", call. = TRUE)
    }else if(!all(exists("negative", where=.ctrls), exists("positive", where=.ctrls))){
      if(!exists("positive", where=.ctrls)){stop("in '.ctrls'. Positive control missing! Please provide a positive and a set of negative controls to analyse.", call. = TRUE)}
      else if(!exists("negative", where=.ctrls)){stop("in '.ctrls'. Negative control missing! Please provide a positive and a set of negative controls to analyse.", call. = TRUE)}
    }else{
  ...
  } else if(any(!unlist(strsplit(unlist(.ctrls), "+", fixed = TRUE)) %in% c(ctrlData$Drug, "all"))){ stop("in '.ctrls'. One or more controls missing in the data set. Only the controls ", gsub(",([^,]*)$"," and\\1", paste(unlist(.ctrls[unlist(.ctrls) %in% ctrlData$Drug]), collapse = ", ")), " were found.", call. = TRUE) 
  } else { 
  ...
  # Subset ony data based on the control treatments, excluding any backfillings
  ...
  ctrlData <- do.call(rbind, setNames(lapply(split(ctrlData, list(ctrlData$Combination.ID, ctrlData$Dispensing.Set)), 
                                             function(x) if(!all(x$Drug %in% c(.controls, "Untreated"))){
                                               subset(x, FALSE)}else{x}), NULL))
  
  # Get the data for only the control treatments
  ctrlData <- subset(ctrlData, Drug %in% .controls)
  
  # Reorder the data set
  ctrlData <- ctrlData[with(ctrlData, order(Dispensing.Set, Plate.Number, Destination.Well)),]
  ...
  ctrlData <- ctrlData[with(ctrlData, order(Combination.ID, Dispensing.Set, Plate.Number, Drug)),]
  ...
  .variance <- function(data){
    ...
    for(samplename in unique(ctrlData$Sample)){
      ...
      ctrlDFS[[samplename]][["variance"]][["data"]] <- split(ctrlDFS[[samplename]][["variance"]][["data"]], factor(ave(ctrlDFS[[samplename]][["variance"]][["data"]]$Drug, ctrlDFS[[samplename]][["variance"]][["data"]]$Combination.ID, FUN = function(g) paste(g, collapse = "+")),
                                                                       levels = lapply(unique(sapply(split(ctrlDFS[[samplename]][["variance"]][["data"]], ctrlDFS[[samplename]][["variance"]][["data"]]$Combination.ID), \(x) cbind(x, paste(x$Drug, collapse=""))$Drug)), paste, collapse = "+")))
      
      
      # Filter out controls that have not been selected by the user
      # ctrlDFS[[samplename]][["variance"]][["data"]] <- ctrlDFS[[samplename]][["variance"]][["data"]][names(ctrlDFS[[samplename]][["variance"]][["data"]]) %in% .controls]

      
      # Retrieve all sets of the controls
      # .controls <- names(ctrlDFS[[samplename]][["variance"]][["data"]])
      ...
      # Set factors for individual columns to allow proper sorting
      ctrlDFS[[samplename]][["variance"]][["data"]] = lapply(ctrlDFS[[samplename]][["variance"]][["data"]], function(x){ transform(x, Drug.Concentration = factor(Drug.Concentration, sort(unique(Drug.Concentration))))  } )
      ...
      # -----------------------------------------------------------------------------------------------------------------------------------
      # Data preparation for plotting: extract data based on the type and number of controls
      for(ctrl in .controls){
        cat('\r', " > Plotting ", ctrl, " variance for ", samplename, ".", strrep(" ", 100), '\n', sep = "")
        
        .ctrlDFS <- ctrlDFS[[samplename]][["variance"]][["data"]][[ctrl]]
        
        # Check, if there are more than one dose of the same control
        if(length(grep(ctrl, listofCtrls$NAME)) > 1){
          .ctrlDFS <- by(.ctrlDFS, paste(ctrl, .ctrlDFS$Drug.Concentration), function(sub) list(data=sub), simplify = FALSE)
        }else{
          # Check, if the control is a combination of two individual controls
          if(length(grep(ctrl, listofCtrls$NAME)) == 0){
            .ctrlDFS <- setNames(list(setNames(list(.ctrlDFS), "data")), ctrl)
          }else{
            .ctrlDFS <- by(.ctrlDFS, paste(.ctrlDFS$Drug), function(sub) list(data=sub), simplify = FALSE)
          }
        }
        ...
        for(ctrl in names(.ctrlDFS)){
        
        ctrlDFS[[samplename]][["variance"]][["boxplot"]][[ctrl]] <- 
          ggplot2::ggplot(.ctrlDFS[[ctrl]][["data"]], aes(x = Sample, y = CPS)) +
          geom_point(aes(colour = Sample)) +
          # geom_text(aes(label = .outlier), na.rm = TRUE, hjust = -0.4) +
          geom_boxplot(alpha = 0.2, notchwidth = 2) +
          scale_y_continuous(name = "CPS", labels = function(x) format(x, scientific = TRUE), limits = c(0, yScaleLimit)) +
          # facet_grid(Sample~.) +
          xlab(NULL) +
          ylab("CPS") +
          ggtitle(paste(ctrl, sep = "")) +
          theme(plot.title = element_text(hjust = 0.5), axis.title.y = element_text(margin = unit(c(0, 4, 0, 0), "mm")), legend.position = "none")
      ...
      ggsave(filename = file.path(.saveto, "quality", samplename, "variance", paste(gsub(",([^,]*)$"," &\\1", paste(names(ctrlDFS[[samplename]][["variance"]][["boxplot"]]), collapse = ", ")), "variance.png", sep = " ")), g, device = "png", width = 69*length(g), height = 126, units = "mm", dpi = 300, limitsize = FALSE)
      ...
      for(ctrl in .controls){
        cat('\r', " > Plotting \'", ctrl, "\' variance by plate for ", samplename, ".", '\n', sep = "")
        ...
        ctrlDFS[[samplename]][["variance"]][["boxplot-byplate"]][[ctrl]] <- 
          ...
          geom_point(aes(colour = Drug.Concentration, fill = Drug.Concentration)) +
          scale_colour_manual(name = "Concentration:", labels = paste(levels(ctrlDFS[[samplename]][["variance"]][["data"]][[ctrl]]$Drug.Concentration), unique(ctrlDFS[[samplename]][["variance"]][["data"]][[ctrl]]$Unit)), values = rep(rainbow(length(names(ctrlDFS[[samplename]][["variance"]][["data"]])), start = 0.6, end = 0.9)[grep(ctrl, names(ctrlDFS[[samplename]][["variance"]][["data"]]), fixed = TRUE)], ifelse(grepl("+", ctrl), 2, 1))) +
          # geom_text(aes(label = .outlier), na.rm = TRUE, hjust = -0.4) +
          geom_text_repel(aes(label = .outlier), stat = "unique", na.rm = TRUE, force = 1, direction = "both", point.padding = unit(0.1, "lines"), box.padding = unit(0.1, "lines"),
                          segment.size = 0.005, segment.color = "black", nudge_x = 0.2, nudge_y = 0, hjust = 0, size = 2.5) +
          geom_boxplot(alpha = 0.2, notchwidth = 2) +
          scale_y_continuous(name = "CPS", labels = function(x) format(x, scientific = TRUE), limits = c(0, yScaleLimit)) +
          facet_grid(Sample~.) +
          xlab(NULL) +
          ylab("CPS") +
          ggtitle(paste(ctrl, sep = "")) +
          guides(fill = "none", colour = guide_legend(keywidth = 0.1, keyheight = 0.1, default.unit = "mm")) +
          labs(tag = paste("Plate:", sep = " ")) +
          theme(plot.title = element_text(hjust = 0.5), legend.position = ifelse(all(!grepl("\\+", ctrl), length(levels(ctrlDFS[[samplename]][["variance"]][["data"]][[ctrl]]$Drug.Concentration)) > 1), "top", "none"), 
                legend.justification = c(0.5, 0), legend.spacing.x = unit(0.1, "mm"), legend.text = element_text(size = 8), 
                legend.key = element_rect(fill = "white"), legend.title = element_text(color = "black", size = 8), legend.margin = margin(b = -5),
                axis.title.y = element_text(margin = unit(c(0, 4, 0, 0), "mm")),
                text = element_text(size = 10), axis.text.x = element_text(angle = 0, vjust = 1, hjust=0.5),
                plot.tag = element_text(hjust = 1, size = 8), plot.tag.position = c(0.08, 0.020))
        
        ggsave(filename = file.path(.saveto, "quality", samplename, "variance", paste(ctrl, "byPlate.png", sep = "-")), device = "png", width = 48*max(as.numeric(levels(ctrlDFS[[samplename]][["variance"]][["data"]][[ctrl]]$Plate.Number)))*sqrt(2), height = 126, units = "mm", dpi = 300, limitsize = FALSE)
      }
      ...
      g <- gridExtra::grid.arrange(grobs = ctrlDFS[[samplename]][["variance"]][["boxplot-byplate"]], nrow = length(.controls))
      ggsave(filename = file.path(.saveto, "quality", samplename, "variance", paste(gsub(",([^,]*)$"," &\\1", paste(names(ctrlDFS[[samplename]][["variance"]][["boxplot"]]), collapse = ", ")), "-variance byPlate.png", sep = " ")), g, device = "png", width = 48*max(as.numeric(levels(ctrlDFS[[samplename]][["variance"]][["data"]][[ctrl]]$Plate.Number)))*sqrt(2), height = 64*length(g), units = "mm", dpi = 300, limitsize = FALSE)
  ...
  .zprime <- function(data){
    ...
    for(samplename in unique(ctrlData$Sample)){
      ...
      # .controls <- unlist(.ctrls)
      
      for(negctrl in .controls[!(.controls %in% .ctrls$positive)]){
        
        cat(" > Calculating the mean (μ) and standard deviation (σ) for the controls (c) ", negctrl, " and ", .ctrls$positive, ".", sep = "")
        
        
        
        # Filter out single drug solvents from combination solvents
        if(grepl("+", negctrl, fixed = TRUE)){
          ctrlDFS[[samplename]][["z-factor"]][[negctrl]][["data"]] <- do.call(rbind, setNames(lapply(split(data[[samplename]][["data"]], data[[samplename]][["data"]]$Sample), function(x) x[(duplicated(x["Combination.ID"]) | duplicated(x["Combination.ID"], fromLast = TRUE) | grepl(.ctrls$positive, x$Drug)), ]), NULL))
        }else{
          ctrlDFS[[samplename]][["z-factor"]][[negctrl]][["data"]] <- do.call(rbind, setNames(lapply(split(data[[samplename]][["data"]], data[[samplename]][["data"]]$Sample), function(x) x[!(duplicated(x["Combination.ID"]) | duplicated(x["Combination.ID"], fromLast = TRUE)), ]), NULL))
        }
        
        # Subset data set with positive and negative control
        ctrlDFS[[samplename]][["z-factor"]][[negctrl]][["data"]] <- subset(ctrlDFS[[samplename]][["z-factor"]][[negctrl]][["data"]], Drug %in% c(unlist(strsplit(negctrl, "+", fixed = TRUE)), .ctrls$positive))
        
        
        
        # Create a data set for the calculation of the z'-factor
        .zdata <- ctrlDFS[[samplename]][["z-factor"]][[negctrl]][["data"]]
        
        
        # Combine solvent names based on the combinations of controls 
        if(grepl("+", negctrl, fixed = TRUE)){
        # .zdata <- do.call(rbind, setNames(lapply(split(.zdata, .zdata$Combination.ID), function(x){ if(nrow(x) > 1){ x$Drug <- paste(paste(x$Drug, "(", x$Drug.Concentration, ")", sep = ""), collapse="+") }; return(x) }), NULL))
          .zdata <- do.call(rbind, setNames(lapply(split(.zdata, .zdata$Combination.ID), function(x){ x$Drug <- paste(x$Drug, collapse="+"); return(x) }), NULL))
          .zdata <- do.call(rbind, setNames(lapply(split(.zdata, .zdata$Combination.ID), function(x){ if(nrow(x) > 1){ x$Drug.Concentration <- paste(x$Drug.Concentration, collapse="+") }; return(x) }), NULL))
          .zdata <- do.call(rbind, setNames(lapply(split(.zdata, .zdata$Combination.ID), function(x){ if(nrow(x) > 1){ x$Unit <- paste(x$Unit, collapse="+") }; return(x) }), NULL))
        }
        
        
        # Split data set for each negative control, and attach positive controls to data set
      # .zdata <- setNames(split(subset(.zdata, Drug == negctrl), subset(.zdata, Drug == negctrl)$Drug.Concentration),  paste(unlist(strsplit(unique(subset(.zdata, Drug == negctrl)$Drug.Concentration), "+", fixed = TRUE)), unlist(strsplit(unique(subset(.zdata, Drug == negctrl)$Unit), "+", fixed = TRUE)), sep = "", collapse = " + "))
        .zdata <- split(subset(.zdata, Drug == negctrl), subset(.zdata, Drug == negctrl)$Drug.Concentration)
        .zdata <- lapply(.zdata, function(x){do.call(rbind, list(x, subset(ctrlDFS[[samplename]][["z-factor"]][[negctrl]][["data"]], Drug == .ctrls$positive)))})
        
        # Name the list using the solvent name and concentration
        if(grepl("+", negctrl, fixed = TRUE)){
          names(.zdata) <- paste(paste(unlist(strsplit(negctrl, "+", fixed = TRUE)), unlist(strsplit(names(.zdata), "+", fixed = TRUE)), sep = " "), collapse = " + ")
        }else{
          names(.zdata) <- paste(negctrl, names(.zdata))
        }
        ...
        # Calculate the z'-factor (zprime) for each sub concentration of the negative control
        for(subctrl in names(.zdata)){
          
          .zdata[[subctrl]] <- do.call(rbind, setNames(lapply(split(.zdata[[subctrl]], .zdata[[subctrl]]$Plate.Number), function(x){ x[[paste(".", "score", sep = "")]] <- zprime(x$CPS[which(x$Drug == negctrl)], x$CPS[which(x$Drug == .ctrls$positive)]); return(x) }), NULL))
          #.zdata[[subctrl]] <- .zdata[[subctrl]][!duplicated(.zdata[[subctrl]][ , c("Plate.Number","Dispensing.Set",".score")]),]
          .zdata[[subctrl]] <- subset(.zdata[[subctrl]], !duplicated(subset(.zdata[[subctrl]], select=c(Plate.Number, Dispensing.Set, .score))))
          
          if(grepl("+", subctrl, fixed = TRUE)){
            .zdata[[subctrl]]$Drug <- paste(paste(unlist(strsplit(unique(.zdata[[subctrl]]$Drug), "+", fixed = TRUE)), paste(unlist(strsplit(unique(.zdata[[subctrl]]$Drug.Concentration), "+", fixed = TRUE)), unlist(strsplit(unique(.zdata[[subctrl]]$Unit), "+", fixed = TRUE)), sep = ""), sep = " "), collapse = " + ")
          }else if(grepl("Untreated", subctrl, fixed = TRUE)){
            .zdata[[subctrl]]$Drug <- paste(.zdata[[subctrl]]$Drug, sep = "")
          } else {
            .zdata[[subctrl]]$Drug <- paste(.zdata[[subctrl]]$Drug, paste(.zdata[[subctrl]]$Drug.Concentration, .zdata[[subctrl]]$Unit, sep = ""))
          }
          
          .zdata[[subctrl]]$Plate.Number <- paste("Plate", .zdata[[subctrl]]$Plate.Number)
          .zdata[[subctrl]]$Plate.Number <- factor(.zdata[[subctrl]]$Plate.Number, paste(unique(gsub("[[:digit:]]", "", .zdata[[subctrl]]$Plate.Number)), sort(order(as.numeric(unique(gsub("[^[:digit:]]", "", .zdata[[subctrl]]$Plate.Number))))), sep = ""))
          .zdata[[subctrl]] <- with(.zdata[[subctrl]], .zdata[[subctrl]][order(Plate.Number, decreasing = FALSE),])
          
        }
        
        .zdata <- do.call(rbind, setNames(.zdata, NULL))
        
        # Select specific rows and columns
        .zdata <- .zdata[!grepl("Dispensing.Set|Combination.ID|CAS.number|Drug.Concentration|Unit|Transfer.Volume|Source.Plate.Barcode|Source.Well|Destination.Well|CPS|.outlier", names(.zdata), ignore.case = TRUE)]
        .zdata <- .zdata[!duplicated(.zdata),]
        
        # LEGACY CODE FOR CONTROLS WITH SINGLE CONCENTRATION OF SOLVENT
        ...
        cat('\r', "Plotting the z'-factor score between the controls ", negctrl, " and ", .ctrls$positive, ".", sep = "")
        
        
        ctrlDFS[[samplename]][["z-factor"]][[negctrl]][["plot"]] <- 
          ggplot2::ggplot(.zdata, aes(x = Plate.Number, y = .zdata[,grepl("score", names(.zdata))])) +
          geom_point(aes(colour = .zdata[,grepl("Drug", names(.zdata))]), size = 7, alpha = .zdata[,grepl("score", names(.zdata))]) +
          scale_colour_manual(values = rainbow(length(unique(.zdata$Drug)), start = 0.6, end = 0.9)) +
          ...
          theme(plot.title = element_text(hjust = 0.5), legend.position = "bottom",
                legend.margin=margin(20,0,0,0), legend.box.margin=margin(0,0,0,0),
                legend.title=element_blank(), legend.text = element_text(size = 14),
                legend.key.size = unit(2, "mm"),
                text = element_text(size = 26),
                axis.text.x = element_text(angle = 0, vjust = 1, hjust=0.5),
                plot.tag = element_text(hjust = 1, size = 21), plot.tag.position = c(0.125, 0.043))
        
        ggsave(filename = file.path(.saveto, "quality", samplename, "z-factor", paste(samplename, " z-factor (", negctrl, ").png", sep = "")), device = "png", width = 169*max(as.numeric(gsub("[^[:digit:]]", "", .zdata$Plate.Number))), height = 567, units = "mm", dpi = 300, limitsize = FALSE)
        
        
        ctrlDFS[[samplename]][["z-factor"]][[negctrl]][["score"]] <- .zdata
    ...
    for(samplename in unique(ctrlData$Sample)){
      ...
      ctrlDFS[[samplename]][["first-column"]] <- 
        ggplot2::ggplot(.exwells, aes(x = Plate.Number, y = factor(Row, rev(levels(Row))))) + 
          ...
          geom_hline(yintercept=seq(1.5, length(unique(.exwells$Row))-0.5, 1), lwd=0.2, colour="black") +
          geom_vline(xintercept=if(length(unique(.exwells$Plate.Number)) == 1){0}else{seq(1.5, length(unique(.exwells$Plate.Number))-0.5, 1)}, lwd=0.2, colour="black") +
          # geom_vline(xintercept=ifelse(length(unique(.exwells$Plate.Number)) == 1, 0, seq(1.5, length(unique(.exwells$Plate.Number))-0.5, 1)), lwd=0.2, colour="black") +
