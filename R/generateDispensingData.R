...
#' @param .randomizeDispensing \code{logical}; if TRUE (default), then drug treatments will be randomized across all plates. If FALSE, drug treatments will be dispensed in sequential order. The default is TRUE, in which the drug treatments are randomized.
#' @param .backfilling \code{logical}; if TRUE (default), then single drug treatments will be backfilled with one unit of solvent. If FALSE, no solvent will be added to single drug treatments.
#' @param .probeDispensing \code{logical}; if TRUE, then the dispensing will be probed only and no dispensing data will be generated. The default is FALSE (default), in which probing is being omitted.
#'
...
#' 
#' The argument \emph{.backfilling} allows the backfilling of drug treatments, primarily single drug treatments, with its solvent to reach the same or highest solvent concentration as present in the combination treatment. This ensures that all wells have the same solvent concentration allowing a comparable effect of the solvent across all drug treatments.
#' Due to the importance of having the same solvent concentration across all drug treatments, this argument is set to TRUE by default.
#' 
...
generateDispensingData <- function(listofCombinations, listofDrugs, listofDoses, listofVolumes, listofCtrls, listofStockConcentrations, sourcePlate,
  ...
  if(!is.list(listofCtrls)){
    stop("in 'listofCtrls'. Argument needs to be an object of class data.frame! Please provide a data frame containing essential columns with the name of the control ['Name'], the CAS number ['CAS number'], dose ['Dose'], unit ['Unit'] the volume ['Volume'] at which to dispense the control and the type ['Type'] of control.", call. = TRUE)
  } else if(!all(sapply(c("name", "cas", "dose", "unit", "vol", "type"), function(x) any(grepl(x, names(listofCtrls), ignore.case = TRUE))))){stop("in 'listofCtrls'. List of controls has missing data! Please provide a data frame containing essential columns with the name of the control ['Name'], the CAS number ['CAS number'], dose ['Dose'], unit ['Unit'] the volume ['Volume'] at atwhich to dispense the control and the type ['Type'] of control.", call. = TRUE)
    ...
    colnames(listofCtrls)[grepl("type", names(listofCtrls), ignore.case = TRUE)] <- "TYPE"
  }
  ...
  # Check, if a list of drugs is provided,  with a column containing the solvents for each drug
  if(missing(listofDrugs)){stop("Data missing! Please provide a list of drugs.", call. = TRUE)}
  if(!is.list(listofDrugs)){
    stop("in 'listofDrugs'. Argument needs to be an object of class data.frame! Please provide a data frame containing a column with the drug name ['Name'], a unique drug number ['Number'], the CAS number ['CAS number'] and the solvent ['Solvent'].", call. = TRUE)
  ...
  if (.probeDispensing){
    ...
    # Prepare data from multiple drug combinations in long format to legacy wide format
    listofCombinations <- transform(listofCombinations, Drug.Number = gsub("Drug ", "\\1",  Drug.Number))
    listofCombinations <- reshape(listofCombinations, idvar = "ID", timevar = "Drug.Number", direction = "wide", new.row.names = NULL)
    
    # Create a set of solvent pairs based on the list of combinations
    m <- cbind(listofDrugs$SOLVENT[match(sub("\\:.*", "", listofCombinations$Drug.A), listofDrugs$NAME)], listofDrugs$SOLVENT[match(sub("\\:.*", "", listofCombinations$Drug.B), listofDrugs$NAME)])
    m <- m[(m[,1] != m[,2] & !duplicated(matrix(m[order(row(m), m)], ncol=ncol(m), byrow=TRUE))),, drop=FALSE]
    
    
    # Calculate the number of controls
    .noCtrls <- nrow(listofCtrls) * .ctrlReplicates + ifelse(is.null(.addUntreated), 0, .addUntreated$replicates) + ifelse(nrow(m) > 0, nrow(m) * .ctrlReplicates, 0)
    ...
    cat("Number of unique drug combinations:",      format(nrow(subset(listofCombinations, Drug.A != Drug.B)[!duplicated(subset(listofCombinations, Drug.A != Drug.B)[, !(names(subset(listofCombinations, Drug.A != Drug.B)) == "ID")], fromLast = TRUE), ]), big.mark=" "), fill = TRUE)
    cat("Number of unique single drug treatments:", format(nrow(subset(listofCombinations, Drug.A == Drug.B)[!duplicated(subset(listofCombinations, Drug.A == Drug.B)[, !(names(subset(listofCombinations, Drug.A == Drug.B)) == "ID")], fromLast = TRUE), ]), big.mark=" "), fill = TRUE)
    ...
    cat("Number of drugs: ", with(listofCombinations, length(unique(c(Drug.A, Drug.B)))), ",  Number of doses: ", ifelse(!length(.noDoses$single), 0, .noDoses$single), "/", ifelse(!length(.noDoses$combination), 0, .noDoses$combination), sep = "", fill = TRUE)
  ...
  if(missing(listofStockConcentrations)){stop("Data missing! Please provide a list of stock concentrations.", call. = TRUE)}
  if(!is.list(listofStockConcentrations)){
    stop("in 'listofStockConcentrations'. Argument needs to be an object of class data.frame! Please provide a data frame containing a column with the drug name ['Name'], a unique drug number ['Number'], the stock concentration ['Concentration'] of that drug and the concentration unit ['Unit'].", call. = TRUE)
  ...
  if(missing(.randomizeDispensing)){message("Randomization argument not provided. Using default: Dispensings ARE randomized.")}

  # Check, if drug treatments should be backfilled
  if(missing(.backfilling)){message("Backfilling argument not provided. Using default: Drug treatments ARE backfilled.")}
  ...
  dispensingData <- data.frame(Drug.1 = do.call(paste, c(listofCombinations[grep("A", names(listofCombinations), value=TRUE)], sep = ":")),
                               Drug.2 = do.call(paste, c(listofCombinations[grep("B", names(listofCombinations), value=TRUE)], sep = ":")),
  ...
  # Add a set of controls for each combination with different solvents
  # apply(expand.grid(rep(list(c("DMSO", "H2O", "TFA")), 2)), 1, paste0, collapse = "+")
  # t(combn(c("DMSO", "H2O", "TFA"), 2, paste0, collapse = "+"))

  # deprecated! 
  # if(length(unique(listofDrugs$SOLVENT)) > 1){
  # dispensingData <- rbind(dispensingData, do.call(rbind, setNames(lapply(1:nrow(t(combn(unique(listofDrugs$SOLVENT), 2))), function(i){ 
  #   data.frame(Drug.1 = paste(rep(t(combn(unique(listofDrugs$SOLVENT), 2))[i,1], .noPlates, each = .ctrlReplicates), rep(max(ctrlList$DOSE[which(ctrlList$NAME == t(combn(unique(listofDrugs$SOLVENT), 2))[i,1])])/2, .noPlates, each = .ctrlReplicates), rep(ctrlList$UNIT[which(ctrlList$NAME == t(combn(unique(listofDrugs$SOLVENT), 2))[i,1] & ctrlList$DOSE == max(ctrlList$DOSE[which(ctrlList$NAME == t(combn(unique(listofDrugs$SOLVENT), 2))[i,1])]))], .noPlates, each = .ctrlReplicates), sep = ":"),
  #              Drug.2 = paste(rep(t(combn(unique(listofDrugs$SOLVENT), 2))[i,2], .noPlates, each = .ctrlReplicates), rep(max(ctrlList$DOSE[which(ctrlList$NAME == t(combn(unique(listofDrugs$SOLVENT), 2))[i,2])])/2, .noPlates, each = .ctrlReplicates), rep(ctrlList$UNIT[which(ctrlList$NAME == t(combn(unique(listofDrugs$SOLVENT), 2))[i,2] & ctrlList$DOSE == max(ctrlList$DOSE[which(ctrlList$NAME == t(combn(unique(listofDrugs$SOLVENT), 2))[i,2])]))], .noPlates, each = .ctrlReplicates), sep = ":"),
  #              Plate.Number = rep(1:.noPlates, each = .ctrlReplicates),
  #              Combination.ID = NA, 
  #              Transfer.Volume = NA,
  #              CAS.number = NA,
  #              Source.Plate.Barcode = NA,
  ...
  # Updated identification of solvent combinations!
  # Comparing solvent for drug 1 with the corresponding solvent for drug 2, if any solvents are being combined, add a set of controls with non-matching solvents
  if(!identical(listofDrugs$SOLVENT[match(sub("\\:.*", "", dispensingData$Drug.1), listofDrugs$NAME)], listofDrugs$SOLVENT[match(sub("\\:.*", "", dispensingData$Drug.2), listofDrugs$NAME)])){
   
    # Extract all the corresponding solvents for each drug pair 
    m <-  cbind(listofDrugs$SOLVENT[match(sub("\\:.*", "", dispensingData$Drug.1), listofDrugs$NAME)],
                 listofDrugs$SOLVENT[match(sub("\\:.*", "", dispensingData$Drug.2), listofDrugs$NAME)])
    # Remove any NA values before further processing
    m <- na.omit(m)
    # Alternative approach, one-liner
    # unique(subset(transform(m, A = pmin(A, B), B = pmax(A, B)), A != B))
    # Remove same solvents from being combined
    m <- unique(subset(m, m[,1] != m[,2]))
    # Remove any duplicate combination of solvents
    m <- m[!duplicated(t(apply(m, 1, sort))),, drop=FALSE]
    # m[!duplicated(matrix(m[order(row(m), m)], ncol=ncol(m), byrow=TRUE)),, drop=FALSE]
    

    # Check, whether solvent pairs are listed in the list of controls, in order for them to be dispensed
    if(!all(as.vector(t(m)) %in% listofCtrls$NAME)){
      stop('\r', paste("WARNING: Adding solvent as a control failed. Solvent ", paste(setdiff(as.vector(t(df)), listofCtrls$NAME), collapse = ", "), " not found in the list of controls.", " Make sure the solvent can be dispensed!", sep = ""), call. = FALSE)
    }
    
     dispensingData <- rbind(dispensingData, do.call(rbind, setNames(lapply(1:nrow(m), function(i){ 
      data.frame(Drug.1 = paste(rep(m[i,1], .noPlates, each = .ctrlReplicates), rep(max(ctrlList$DOSE[which(ctrlList$NAME == m[i,1])])/2, .noPlates, each = .ctrlReplicates), rep(ctrlList$UNIT[which(ctrlList$NAME == m[i,1] & ctrlList$DOSE == max(ctrlList$DOSE[which(ctrlList$NAME == m[i,1])]))], .noPlates, each = .ctrlReplicates), sep = ":"),
                 Drug.2 = paste(rep(m[i,2], .noPlates, each = .ctrlReplicates), rep(max(ctrlList$DOSE[which(ctrlList$NAME == m[i,2])])/2, .noPlates, each = .ctrlReplicates), rep(ctrlList$UNIT[which(ctrlList$NAME == m[i,2] & ctrlList$DOSE == max(ctrlList$DOSE[which(ctrlList$NAME == m[i,2])]))], .noPlates, each = .ctrlReplicates), sep = ":"),
                 Plate.Number = rep(1:.noPlates, each = .ctrlReplicates),
                 Combination.ID = NA, 
                 Transfer.Volume = NA,
                 CAS.number = NA,
  ...
  dispensingData <-  transform(dispensingData, Transfer.Volume = ifelse(Drug %in% ctrlList$NAME & is.na(Transfer.Volume), ctrlList$VOLUME[match(paste(Drug, Drug.Concentration), paste(ctrlList$NAME, ctrlList$DOSE/2))]/2, Transfer.Volume))
  ...
  # BACKFILLING: Adding DMSO to each well that has only a single drug dispensed
  # Note: this is done to have the same DMSO concentration in each well,
  # Additional Note: Single drug treatments with H2O are backfilled to 0.1% DMSO,
  # while single drug treatments with DMSO to 0.2% DMSO
  
  if(.backfilling == TRUE){
  
  # Add solvent information for each drug to the dispensing data
  dispensingData$Solvent <- listofDrugs$SOLVENT[match(dispensingData$Drug, listofDrugs$NAME)]
  
  # Count the number of drugs per combination and add for the remaining number of drugs one unit of the transfer volume with the corresponding solvent
  # Each drug is backfilled with one unit of the same solvent it is dissolved in
  # For H2O single drug treatments, one unit (1 * 5 nl) of H2O is added
  # For DMSO single drug treatments, one unit (1 * 5 nl) of DMSO is added
  # Note: no backfilling for drugs in single drug treatments that are dispensed at the highest concentration
  # Note: transfer volume of backfilled solvent corresponds to the transfer volume of the drug 
  ...
  dispensingData <- do.call(rbind, lapply(split(dispensingData, dispensingData$Combination.ID), 
                                          function(x) rbind(x, if(nrow(x) < 2 & !any(x$Drug %in% c(ctrlList$NAME, if(!is.null(.addUntreated)){.addUntreated$name})) & 
                                            ...
                                            within(x[rep(1, each = 1),], {Drug = x$Solvent; CAS.number = ctrlList$CAS_NUMBER[match(x$Solvent, ctrlList$NAME)];
  ...
  .noSingTreatments <- as.numeric(nrow(subset(listofCombinations, Drug.A == Drug.B)[!duplicated(subset(listofCombinations, Drug.A == Drug.B)[, !(names(subset(listofCombinations, Drug.A == Drug.B)) == "ID")], fromLast = TRUE), ]))
  .noDrugs <-          as.numeric(with(listofCombinations, length(unique(c(Drug.A, Drug.B)))))
