...
consolidateData <- function(dispensingData, rawMeasurements, .barcodeReference){
  ...
  for (i in names(rawMeasurements)) {
    ...
    plateBarcode <- i
    destinationPlateID <- ifelse("dispensingID" %in% names(dispensingData), dispensingData$dispensingID, unique(.barcodeReference$PlateID))
    plateSet <- gsub("\\d+", "", gsub(ifelse("dispensingID" %in% names(dispensingData), dispensingData$dispensingID, unique(.barcodeReference$PlateID)), "", plateBarcode))
    plateNumber <- as.numeric(gsub("\\D+", "", gsub(ifelse("dispensingID" %in% names(dispensingData), dispensingData$dispensingID, unique(.barcodeReference$PlateID)), "", plateBarcode)))
    ...
    dfs[[plateBarcode]]$Well <- gsub("(?<![0-9])0+", "", dfs[[plateBarcode]]$Well, perl = TRUE)
    levels(dfs[[plateBarcode]]$Well) <- dfs[[plateBarcode]]$Well
